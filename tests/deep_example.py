...
from kadapt.models.deep_kernel.deep_adaptation import deep_full_adaptation
...
dfa_model = deep_full_adaptation(config_files, gpu_flg)
dfa_model.fit(source_traindata, target_traindata, split, verbose)

#train target model

target_model = deep_full_adaptation(config_files, gpu_flg)
...
eval_list.append({'task': 'source adapt to target', 'predict error': adapt_error})


#adaptation error
predy = target_model.predict(source_testx, 'adapt')
adapt_error = target_model.score(predy, source_testy).item()
eval_list.append({'task': 'target adapt to source', 'predict error': adapt_error})
