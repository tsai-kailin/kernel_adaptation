...
from kadapt.models.plain_kernel.multienv_adaptation import multienv_adapt
...
def gen_batch_data(n_env, n, mean_z, sigma_z, sd_list):
    data_list = []
    for env_id, sd in enumerate(sd_lst[:n_env]):
        ...
        Z = ((jnp.asarray(gen_Z(n, mean_z[env_id], sigma_z[env_id], key=keyz))))
        U = ((jnp.asarray(gen_U(Z, n, key=subkeysu))).T)
...
source_train = gen_batch_data(n_env, n, mean_z, sigma_z, sd_train_list)

#test set only has 1000 samples
sd_test_list = sd_lst[n_env:n_env*2]
source_test = gen_batch_data(n_env, 1000, mean_z, sigma_z, sd_test_list)
...
target_train = gen_batch_data(1,    n, target_mean_z, target_sigma_z, [5949])
target_test  = gen_batch_data(1, 1000, target_mean_z, target_sigma_z, [5654])
...
split = True
scale = 1
estimator = multienv_adapt(source_train, target_train, source_test, target_test, split, scale, lam_set, method_set, kernel_dict)
