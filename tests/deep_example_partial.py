...
from kadapt.models.deep_kernel.deep_adaptation import deep_partial_adaptation
from kadapt.data.gen_demand import generate_demand_dataset
...
config_files = './configs/demand_partial_config.json'
...
def gen_u_target(n_sample: int, rng):
    demand = rng.uniform(0, 1, n_sample)     #U
    return demand
...
dfa_model = deep_partial_adaptation(config_files, gpu_flg)
dfa_model.fit(source_traindata, target_traindata, split, verbose)

#train target model

target_model = deep_partial_adaptation(config_files, gpu_flg)
...
eval_list.append({'task': 'source adapt to target (m0)', 'predict error': adapt_error})


#adaptation error
predy = target_model.predict(source_testx, 'adapt')
adapt_error = target_model.score(predy, source_testy).item()
eval_list.append({'task': 'target adapt to source (m0)', 'predict error': adapt_error})
